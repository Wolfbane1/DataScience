...
#             , y = 0.02, colour="red", size=3, label = " <---- Percentiles 25% y 75% ----> ") + 
    labs(y="%", title="Probabilidad de un CRG-base[5 dígitos] dada una Edad")
    print(g)
  }  
  rm(anyo)
  rm(dx)
...
```{r results='asis', echo=FALSE, errors=FALSE, messages=FALSE, warnings=FALSE, eval=TRUE}
  cat("\n\n<b>Gráfica 7.3 - Probabilidad de la Edad[rango] dado un CRG</b>\n\n")
  colorCRG_3 <- c("thistle","green", "lemonchiffon", "lemonchiffon", "red1", "lemonchiffon", "green1",
                  "red2", "blue", "grey")
...
for (anyo in unique(csv$Anyo)) {
  ...
  for (crg in unique(atc$CRG)) {
    ...
    #p <- atc_col[atc_col >= quantile(atc_col, 0.96)]
    p <- atc_col
    #n <- names(atc_col[atc_col >= quantile(atc_col, 0.96)])
    n <- names(atc_col)
    a <- rep(anyo, length(p))
    c <- rep(crg, length(p))
...
m <- as.data.frame(m)
colnames(m) <- c("Anyo", "CRG", "Names", "Prob")
m$Anyo <- as.factor(m$Anyo)
m$CRG <- as.factor(m$CRG)
m$Prob <- round(as.numeric(as.character(m$Prob)), 4)
atc <- subset(m, m$CRG %in% c(5424, 6144, 7071) & m$Prob >= 3.0 )  

#Ahora hay que concatenar las columnas que no estén en los otros CRGs y que estén en el previo.
u <- unique(atc$Names)
atc_f <- subset(m, m$CRG %in% c(5424, 6144, 7071) & m$Names %in% u)

t <- subset(atc_f, atc_f$Anyo==2011)
#t$CRG <- as.factor(as.character(t$CRG))
t$CRG <- as.integer(as.character(t$CRG))
t$Prob <- as.numeric(t$Prob)

par(mfrow=c(1,2), mar=c(3,2,3,2))
plot(t$CRG, t$Prob, type ="p", xlim=c(5000, 7500))
title(main="Año 2011 - Evolución de ATCs por CRG-base", cex.main=0.7)
segments(t[t$CRG==5424,"CRG"], t[t$CRG==5424, "Prob"], t[t$CRG==6144,"CRG"], t[t$CRG==6144,"Prob"])
segments(t[t$CRG==6144,"CRG"], t[t$CRG==6144, "Prob"], t[t$CRG==7071,"CRG"], t[t$CRG==7071,"Prob"])
text(rep(5200,length(t[t$CRG==5424, "Prob"])), t[t$CRG==5424, "Prob"], t[t$CRG==5424, "Names"],cex=0.7 )
text(rep(7300,length(t[t$CRG==7071, "Prob"])), t[t$CRG==7071, "Prob"], t[t$CRG==7071, "Names"],cex=0.7 )

t <- subset(atc_f, atc_f$Anyo==2012)
#t$CRG <- as.factor(as.character(t$CRG))
t$CRG <- as.integer(as.character(t$CRG))
t$Prob <- as.numeric(t$Prob)

plot(t$CRG, t$Prob, type ="p", xlim=c(5000, 7500))
title(main="Año 2012 - Evolución de ATCs por CRG-base", cex.main=0.7)
segments(t[t$CRG==5424,"CRG"], t[t$CRG==5424, "Prob"], t[t$CRG==6144,"CRG"], t[t$CRG==6144,"Prob"])
segments(t[t$CRG==6144,"CRG"], t[t$CRG==6144, "Prob"], t[t$CRG==7071,"CRG"], t[t$CRG==7071,"Prob"])
text(rep(5200,length(t[t$CRG==5424, "Prob"])), t[t$CRG==5424, "Prob"], t[t$CRG==5424, "Names"],cex=0.7 )
text(rep(7300,length(t[t$CRG==7071, "Prob"])), t[t$CRG==7071, "Prob"], t[t$CRG==7071, "Names"],cex=0.7 )
...
#plot(t$CRG, t$Prob)
#
#   g <- ggplot(t, aes(x=CRG, y=Prob, col=Anyo)) + geom_point() + 
# #     scale_fill_manual(values=colorAnyo) +
#      theme(axis.text.x = element_text(size = 7, colour = "black")) + 
#      theme(title = element_text(size = 7, colour = "black")) + 
#      labs(x="CRG-base", y="Probabilidad",
#         title=paste("Evolución de probabilidad de ocurrencia de ATCs", anyo, sep=""))
#    suppressMessages(print(g))
#
...
#Probabilidad de ocurrencia de un medicamento (normalizando a partir del número de pacientes)
...
m$CRG <- as.factor(m$CRG)
m$Prob <- round(as.numeric(as.character(m$Prob)), 4)
m$PosX <- ifelse(m$CRG == 5424, 1, m$PosX)
m$PosX <- ifelse(m$CRG == 6144, 2, m$PosX)
#m$PosX <- ifelse(m$CRG == 7070, 3, m$PosX)
m$PosX <- ifelse(m$CRG == 7071, 3, m$PosX)
atc <- subset(m, m$CRG %in% c(5424, 6144, 7071) & m$Prob >= 25.0 )  
...
for (anyo in unique(m$Anyo)) {
  ...
  plot(t5424$PosX, t5424$Prob*2, type ="p", ylim=c(0, 170), xlim=c(0.5, 3.5), yaxt="n", xaxt="n", pch=1)
  ...
  axis(2, at = c(20, 40, 60, 80, 100, 120, 140, 160), 
       labels=c(10, 20, 30, 40, 50, 60, 70, 80), cex.axis=0.6)
  title(main="Año 2011 - Evolución de ATCs por CRG-base", cex.main=0.7)
  for(i in 1:8) {
    text(0.8, t5424[i,"Prob"]*2, t5424[i, "Names"], cex=0.7)
  ...
  for(i in 9:15) {
    text(3.2, t7071[i,"Prob"]*2, t7071[i, "Names"], cex=0.7)
